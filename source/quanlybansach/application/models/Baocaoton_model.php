...
class Baocaoton_model extends CI_Model
{
	public $table ='sach';
	...
	public function get_AllSach()
	{
		...
		public function get_DanhsachPhanTrang($limit, $start)
		{
			...
			$this->db->from('baocaoton');// lấy trong bảng sách
			$this->db->join('sach', 'baocaoton.masach = sach.masach');
			$query = $this->db->get();
			return $query->result_array();
		}
		public function record_count()
		{
			return $this->db->count_all("baocaoton");
		}

		public function insertBaoCaoTon($data)
		{
			$this->db->insert_batch('baocaoton', $data);
			$afftectedRows=$this->db->affected_rows();
			if($afftectedRows >0)
				return 1;
			return 0;
